...
def login():
    ...
    if request.method == 'POST':
        ...
        if not u or not p:
            ...
            cursor = conn.cursor()
            try:
                cursor.execute('SELECT NguoiBan, VaiTro FROM NguoiDung WHERE TenDangNhap=? AND MatKhau=?', (u, p))
...
def quen_mat_khau():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('SELECT * FROM NguoiDung WHERE TenDangNhap=? AND NguoiBan=?', (u, f))
        user = cursor.fetchone()
