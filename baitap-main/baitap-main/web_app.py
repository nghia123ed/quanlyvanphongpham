...
def xoa_sp(ma_sp):
    ...
    try:
        ...
        db_execute("DELETE FROM Products WHERE ID = ?", (ma_sp,), commit=True)
        return redirect(url_for('kho_hang'))
...
def thanh_toan():
    ...
    try:
        ...
        if customer:
            try:
                cursor.execute('INSERT INTO Invoices (InvoiceID, Seller, CreatedDate, TotalAmount, Customer) VALUES (?,?,?,?,?)', 
...
def lich_su():
    ...
    try:
        cursor.execute('SELECT InvoiceID, Seller, CreatedDate, TotalAmount, Customer FROM Invoices ORDER BY InvoiceID DESC')
        hd = cursor.fetchall()
    except Exception:
...
def api_thong_ke():
    ...
    sql = 'SELECT CONVERT(date, CreatedDate) AS d, SUM(TotalAmount) FROM Invoices '
    params = []
    if start and end:
        sql += 'WHERE CONVERT(date, CreatedDate) BETWEEN ? AND ? '
        params = [start, end]
    sql += 'GROUP BY CONVERT(date, CreatedDate) ORDER BY CONVERT(date, CreatedDate)'
    try:
        cursor.execute(sql, params)
        rows = cursor.fetchall()
